...
Frontier AI risk is not accumulating neatly inside the categories used to govern it. It is evolving across them. As frontier systems become more capable, more agentic, and more deeply embedded in social and institutional life, harms that are often treated as distinct may begin to interact in ways that are analytically and operationally consequential. Cyber misuse, CBRN misuse, disinformation, autonomous escalation, radicalization, parasocial manipulation, and mental-health harms do not necessarily remain confined to their own lanes. They may propagate through shared technical, institutional, and psychological pathways, altering the broader environment in which other risks emerge and are managed.

Yet many current frontier safety approaches still organize risk in piecemeal fashion. Major frameworks have advanced the field by defining categories, thresholds, and deployment gates for severe harms, but they usually evaluate and mitigate those harms on separate tracks \citep{anthropic2024rsp,openai2023preparedness,deepmind2024fsf,nist2023rmf,iaisr2025}. That structure offers administrative clarity, and in some settings it is plainly useful. But it also rests on an assumption that deserves more scrutiny than it has received: that frontier risks can be meaningfully assessed, and meaningfully reduced, as if they were largely independent.

This paper asks whether that assumption may itself be wrong. The concern is not only that current frameworks may overlook some harms. It is that they may mis-specify the structure of the risk landscape. If multiple frontier-AI risk domains draw on the same finite institutional and epistemic resources, then stresses in one domain may degrade response capacity in others. Under those conditions, disinformation may not simply be a separate social harm; it may erode trust, coordination, and monitoring quality in ways that amplify cyber misuse or CBRN misuse. Radicalization may increase the number of actors motivated to exploit dangerous capabilities. Parasocial manipulation and mental-health harms may alter vulnerability, compliance, and social resilience. In a coupled sociotechnical system, these dynamics do not sit beside catastrophic risks. They may help generate them.

Consider a simple example. A frontier model with improved coding capabilities helps a moderately skilled actor produce phishing infrastructure at scale. That is usually treated as a cyber risk. The same or a related model helps another actor reason through precursor acquisition or synthesis pathways relevant to CBRN misuse. That is usually treated as a separate risk. Now add a third dynamic: AI-enabled disinformation that degrades trust in public institutions, overwhelms fact-checking capacity, and intensifies grievance narratives. In many current frameworks, these still appear as separate problem sets. In practice, however, they may converge on the same shared substrate: the analysts who investigate abuse, the institutions responsible for response, the public trust needed for coordinated mitigation, and the political capacity required to sustain governance. The severity of each risk then depends not only on model capability in isolation, but also on the condition of the environment receiving it.

That is the modeling premise of this paper. We do not claim to have built a predictive world model of frontier-AI catastrophe. We claim something narrower, but still important: if cross-domain dependence is real, then siloed models are likely to miss amplification pathways, shared bottlenecks, and mitigation tradeoffs that only become visible when risks are analyzed together. The problem is therefore not just evaluative. It is strategic. A piecemeal mitigation regime may invest heavily in domain-specific controls while underinvesting in the shared substrates that determine whether the overall system remains governable under pressure.

To study that problem, we use system dynamics (SD) \citep{forrester1961,sterman2000,meadows2008}. SD is well suited to questions of this kind because the core hypothesis is about feedback, delay, accumulation, depletion, and endogenous escalation. The relevant object is not a static hazard label but an evolving system state. We therefore represent frontier risk at three connected levels: causal loop diagrams that capture feedback structure, stock-and-flow models that simulate dynamic behavior, and formal graph representations that enable structural comparison across domains. We build five domain-specific models covering cyber misuse, CBRN misuse, deception and manipulation, autonomous escalation, and governance failure; integrate them into a cross-domain supergraph with shared intermediary nodes; and then test whether the coupled representation reveals structural and dynamic features that siloed models omit.

\paragraph{Contributions.}
The paper makes four main contributions. First, it formalizes a coupling hypothesis for frontier AI risk, arguing that risks often assessed separately may become dependent through shared sociotechnical substrates. Second, it implements that hypothesis as a reproducible systems-dynamics framework spanning five domains and an integrated cross-domain supergraph. Third, it shows within the implemented model family that coupled representations can reveal amplification pathways, shared bottlenecks, and lower-tier harm effects that siloed representations omit. Fourth, it extends the framework into a partially observable control setting, allowing mitigation to be studied not only as domain-specific risk reduction but as preservation of system stability and controllability under pressure.

This also helps clarify what the paper is and is not arguing. It is not a claim that system dynamics is the only valid way to study frontier risk, nor that all lower-tier harms should automatically be treated as catastrophic. It is a claim that dynamic, cross-domain dependence may be a load-bearing feature of the risk landscape, and that frameworks organized around isolated domains may therefore understate both compound risk and the difficulty of maintaining control. If that is true even in part, then frontier risk assessment should move beyond piecemeal scorekeeping toward models that can represent how harms interact over time.

The remainder of the paper develops that argument in stages. Section~\ref{sec:related} situates the approach within adjacent literatures and explains why recent work in dynamical modeling, control, and agentic evaluation strengthens the case for cross-domain analysis. Section~\ref{sec:methodology} operationalizes the coupling hypothesis as a reproducible systems-dynamics framework. The later sections then test whether the integrated representation exposes amplification pathways, shared bottlenecks, and control-relevant mitigation tradeoffs that siloed models are liable to miss.
%==============================================================================
...
TODO: NIST AI RMF \citep{nist2023rmf}. ????? GPAI Code of Practice, ISO 42001, etc. 

\subsection{Frontier Safety Frameworks}

The current generation of frontier AI safety frameworks includes Anthropic's Responsible Scaling Policy (RSP) \citep{anthropic2024rsp}, OpenAI's Preparedness Framework \citep{openai2023preparedness}, DeepMind's Frontier Safety Framework \citep{deepmind2024fsf}. 
...
\subsection{Computational Approaches to AI Risk Assessment}

Recent work has explored structured methods for quantifying AI safety risks. Wisakanto et al.\ \citep{wisakanto2025adaptingprobabilisticriskassessment} adapt probabilistic risk assessment (PRA) techniques from high-reliability industries (nuclear, aerospace) to AI, introducing aspect-oriented hazard analysis, risk pathway modeling through causal chains, and uncertainty management via scenario decomposition and reference scales, implemented as a workbook tool for developers and regulators. Murray et al.\ \citep{murray2025methodologyquantitativeairisk} propose a six-step quantitative risk modeling methodology that decomposes risk scenarios into quantifiable parameters, estimates baseline risk without AI, identifies key risk indicators from benchmarks, and aggregates them via Monte Carlo simulation into concrete probabilistic claims. Barrett et al.\ \citep{barrett2025quantitativemodelingcybersecurityrisks} apply that methodology to AI-enabled cyber offense, developing nine detailed risk models using the MITRE ATT\&CK framework with Delphi-elicited and LLM-simulated expert estimates of AI uplift. Jackson et al.\ \citep{jackson2026riskthresholdsaienabledcyber} propose Bayesian networks as a tool for operationalizing AI cyber risk thresholds, demonstrating through an AI-augmented phishing case study how qualitative threat descriptions can be decomposed into measurable probabilistic variables. Barrett and Baum \citep{barrett2023} earlier used fault trees and influence diagrams to model pathways to AI catastrophe.

These approaches share a common property: they model individual risk domains (cyber, CBRN, manipulation) as separate assessment targets with independent causal chains. Our contribution is distinct in that we model the coupling between risk domains through shared sociotechnical stocks (trust erosion, institutional overload, detection degradation), providing a full pipeline from qualitative causal structure through dynamic simulation to cross-domain convergence stress testing.
...
% Question: why is it called barrett2023 when it was published back in 2017?
...
\bibitem[Barrett et~al.(2025)]{barrett2025quantitativemodelingcybersecurityrisks}
Steve Barrett, Malcolm Murray, Otter Quarks, Matthew Smith, Jakub Kryś, Siméon Campos, Alejandro Tlaie Boria, Chloé Touzet, Sevan Hayrapet, Fred Heiding, Omer Nevo, Adam Swanda, Jair Aguirre, Asher Brass Gershovich, Eric Clay, Ryan Fetterman, Mario Fritz, Marc Juarez, Vasilios Mavroudis, and Henry Papadatos.
\textit{Toward Quantitative Modeling of Cybersecurity Risks Due to AI Misuse}, 2025.
...
\bibitem[Jackson et~al.(2026)]{jackson2026riskthresholdsaienabledcyber}
Krystal Jackson, Deepika Raman, Jessica Newman, Nada Madkour, Charlotte Yuan, and Evan R. Murphy.
\textit{Toward Risk Thresholds for AI-Enabled Cyber Threats: Enhancing Decision-Making Under Uncertainty with Bayesian Networks}, 2026.
Available at \url{https://arxiv.org/abs/2601.17225}.

% @misc{jackson2026riskthresholdsaienabledcyber,
...
\bibitem[Murray et~al.(2025)]{murray2025methodologyquantitativeairisk}
Malcolm Murray, Steve Barrett, Henry Papadatos, Otter Quarks, Matt Smith, Alejandro Tlaie Boria, Chloé Touzet, and Siméon Campos.
\textit{A Methodology for Quantitative AI Risk Modeling}, 2025.
Available at \url{https://arxiv.org/abs/2512.08844}.

% @misc{murray2025methodologyquantitativeairisk,
...
\bibitem[Wisakanto et~al.(2025)]{wisakanto2025adaptingprobabilisticriskassessment}
Anna Katariina Wisakanto, Joe Rogero, Avyay M. Casheekar, and Richard Mallah.
\textit{Adapting Probabilistic Risk Assessment for AI}, 2025.
Available at \url{https://arxiv.org/abs/2504.18536}.

% @misc{wisakanto2025adaptingprobabilisticriskassessment,
